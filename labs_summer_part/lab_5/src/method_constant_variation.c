...
void first_diff_sys_equations_(double a,
                              double b,
                              double h,
                              ...
                              double second_start_value)
{
    ...
    double z1a = (a0 - b0 * t1) / a0, dz1a = t1;

    double dz2a = t2, z2a = (y0 - b0 * t2) / a0;

    double z1 = t1 + 2, dz1 = t1;
    double z2 = t2 + 2, dz2 = t2;

    double arr1[(int)(b * 10) + 1], arr2[(int)(b * 10) + 1];
    double arr1_s[(int)(b * 10) + 1], arr2_s[(int)(b * 10) + 1];

    int i = 0;

    double t = z1, s = dz1;

    for(double x = (double)0; x <= b; x += h)
    {
        arr1[i] = t;
        arr1_s[i] = s;
        t = t + h * f2(s);
    ...
    double d = a1 * (arr2[(int)(b * 10)] - arr1[(int)(b * 10)]) + b1 * (arr2_s[(int)(b * 10)] - arr1_s[(int)(b * 10)]);
    double c = ((double)1 / d) * (y1 - a1 * arr1[(int)(b * 10)] - b1 * arr1_s[(int)(b * 10)]);

    printf("d = %.4lf\nc = %.4lf\n", d, c);

    printf("---------------------------------------------------------\n");
    printf("| k | x_k |   z1   |   z'1   |   z2   |   z'2  |    y   |\n");

    for(int i = 0; i < (int)(b * 10) + 1; i++)
    {
        printf("---------------------------------------------------------\n");
        printf("| %d | %.1lf | %.4lf | %.4lf  | %.4lf | %.4lf | %.4lf |\n", i, (double)i / (double)10,
                arr1[i], arr1_s[i], arr2[i], arr2_s[i], (1 - c) * arr1[i] + arr2[i] * c);
    }
                              ...
                              double second_start_value)
{
    ...
    {
        printf("------------------------------------------------------------------------------\n");
        printf("| %d | %.1lf | %.4lf | %.4lf | %.4lf  | %.4lf  | %.4lf  | %.4lf  | %.6lf |\n", x, (double)((double)x / 10), 
